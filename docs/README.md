...
- `explainers/application-explainer.html`: product/application overview for readers who want the "what is this?" path.
- `explainers/architecture-explainer.html`: architecture overview for readers who want the "how is this shaped?" path.

## Explainer Pages

All explainer pages live in `explainers/`. They are manually maintained derived summaries for orientation and task handoff only. Markdown docs remain canonical; update the relevant markdown first, then manually realign the explainer pages until a generator exists.
